...
class Test{
	public void RunTests(){
	    TLE_DATA test = new TLE_DATA();
		YAMTRAK y = new YAMTRAK();
		TimeUtils timeutil = new TimeUtils();
		//initialise data
		test.YE = 1990;
        test.TE = 191.145409;
        ...
        test.RV = 1585 ;
		//print info
		Console.WriteLine("Running tests with TLE parameters:");
		Console.WriteLine("YE: " + test.YE);
		Console.WriteLine("TE: " + test.TE);
		Console.WriteLine("IN: " + test.IN);
		Console.WriteLine("RA: " + test.RA);
		Console.WriteLine("EC: " + test.EC);
		Console.WriteLine("WP: " + test.WP);
		Console.WriteLine("MA: " + test.MA);
		Console.WriteLine("MM: " + test.MM);
		Console.WriteLine("M2: " + test.M2);
		Console.WriteLine("RV: " + test.RV);
		
		//covvert angle to radians
		test.toRads();
		//compute result vectors
		Vectors result = y.satvec(test, timeutil.UnixTimeToDayNumber(657590400),timeutil.UnixTimeToDayFrac(657590400));
		//compare against BASIC program results
		if(!(Math.Abs(result.position.x) - 3597.164807 <= 0.000001)){
        Console.WriteLine("error Sx: " + result.position.x);
        }
        
        if(!(Math.Abs (result.position.y) - 14599.945116 <= 0.000001)){
        Console.WriteLine("error Sy: "+ result.position.y);
        }
        
        if(!(Math.Abs (result.position.z) - 4108.495425 <= 0.000001)){
        Console.WriteLine("error Sz: " + result.position.z);
        }
        
        if(!(Math.Abs (result.velocity.x) -1.444712  <= 0.000001)){
        Console.WriteLine("error Vx: " + result.velocity.x);
        }
        
        if(!(Math.Abs (result.velocity.y ) - 5.119617  <= 0.000001)){
        Console.WriteLine("error Vy: " + result.velocity.y);
        }
        
        if(!(Math.Abs(result.velocity.z) - 2.717625  <= 0.000001)){
        Console.WriteLine("error Vz: " + result.velocity.z);
        }
...
class MainClass{
	public static void Main (string[] args){
		...
		//example 
		YAMTRAK y = new YAMTRAK();
		y.calculate(1360085589 , "1 37853U 11061D   13055.55523038  .00001904  00000-0  15445-3 0  4351", "2 37853 101.7127 217.1109 0239318 349.9095   9.7345 14.82390046 71758"
);
		double[] pos = y.GetPos();
		double[] vel = y.GetVel();
		System.Console.WriteLine("Sx: " + pos[0]);
		System.Console.WriteLine("Sy: " + pos[1]);
		System.Console.WriteLine("Sz: " + pos[2]);
		System.Console.WriteLine("Vx: " + vel[0]);
		System.Console.WriteLine("Vy: " + vel[1]);
		System.Console.WriteLine("Vz: " + vel[2]);
		System.Console.WriteLine("Distance: " + (Math.Sqrt(Math.Pow(pos[0],2) + Math.Pow(pos[1],2) + Math.Pow(pos[2],2))));
		System.Console.WriteLine("Velocity: " + (Math.Sqrt(Math.Pow(vel[0],2) + Math.Pow(vel[1],2) + Math.Pow(vel[2],2))));
	}
}*/