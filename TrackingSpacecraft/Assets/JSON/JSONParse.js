...
class json {
    ...
    var keys:  List.<String> = new List.<String>(1);
    ...
    function _push(val: json):json{
        ...
        //fixed warning by using for loop to coppy v into values
        for (var i = 0; i < v.length; i++){
        	if(i < values.Count){
        		values[i] = v[i] as json;
        	}else{
        		values.Insert(i, v[i] as json);
        	}
        }
    ...
    function _set(key: String, value: json): json {
        if (type == jsonType._object) {
            ...
            k.push(key);
            v.push(value);
			//used for loop to coppy k to keys removing error
            for(var j = 0; i < k.length; i++){
            	if(i < keys.Count){
            		keys[i] = k[i] as String;
            	}else{
            		keys.Insert(i, k[i] as String);
            	}
            }
        	...
        	for (var l = 0; l < v.length; l++){
        		if(l < values.Count){
        			values[l] = v[l] as json;
        		}else{
        			values.Insert(l, v[l] as json);
        		}
        	}
...
private static function number(): json {
    ...
    if (System.Double.IsNaN(number)) {
    	...
    	//converted to double via string
    	var numStr = number.ToString();
        return json._number(double.Parse(numStr));
    }
